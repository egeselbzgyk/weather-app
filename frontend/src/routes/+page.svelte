  ...
  async function loadCurrentWeather(location) {
    ...
    try {
      ...
      weatherData.forEach((item) => {
        if (
          !latest[item.attribute] ||
          new Date(item.timestamp) > new Date(latest[item.attribute].timestamp)
        ) {
...
<div class="weather-app">
  ...
  {:else if error}
    <div class="error">
      <p>❌ {error}</p>
      <button on:click={() => loadCurrentWeather(selectedLocation)}
        >Try Again</button
      >
    </div>
  {:else if currentWeather}
    <div class="weather-card">
      ...
      <div class="weather-grid">
        <div
          class="weather-item main-temp"
          style="border-left-color: {getTemperatureColor(
      ...
      <div class="refresh-section">
        <button
          class="refresh-btn"
          on:click={() => loadCurrentWeather(selectedLocation)}
        >
          🔄 Refresh Data
        </button>
      </div>
